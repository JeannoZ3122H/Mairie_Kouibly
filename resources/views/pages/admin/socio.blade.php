            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a  href="{{ route('socio_cult') }}">
                            <h3 class="title">Socio culturel</h3>
                        </a>
                    </li>
            ...
            <div class="row justify-content-center mt-3">
                @if (isset($get_socio))
                    <div class="col-lg-6 col-md-12 col-sm-8">
                        <form action="{{ route('update_socio',$get_socio->id) }}" method="post" enctype="multipart/form-data">
                            
                            @csrf
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label for="exampleFormControlInput1" class="form-label">RESPONSABLE</label>
                                    <input type="text" value="{{ old('responsable') ?? $get_socio->responsable }}" name="responsable" class="form-control" id="exampleFormControlInput1" placeholder="">
                                </div>
                                <br>
                    ...
                    <div class="table-responsive">
                        @if (isset($sociaux))
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    @foreach ($sociaux as $items)
                                        <tr>
                                            <td>{{ $items->responsable }}</td>
                                            <td width="1000">{!! $items->description !!}</td>
                                            
                                            <td class="text-center d-flex">
                                                <a href="{{ route('edit_socio_cult', $items->id) }}" class="btn btn-success">
                                                    Modifier
                                                </a>
                                                <form action="{{ route('destroy_socio_cult', $items->id) }}" method="post">
                                                    @csrf
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Socio culturel</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{ route('add_socio_cult') }}" method="post" enctype="multipart/form-data">
