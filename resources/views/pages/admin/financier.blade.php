            ...
            <div class="row justify-content-center mt-3">
                @if (isset($get_financier))
                    <div class="col-lg-6 col-md-12 col-sm-8">
                        <form action="{{ route('update_financier',$get_financier->id) }}" method="post" enctype="multipart/form-data">
                            
                            @csrf
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label for="exampleFormControlInput1" class="form-label">RESPONSABLE</label>
                                    <input type="text" value="{{ old('responsable') ?? $get_financier->responsable }}" name="responsable" class="form-control" id="exampleFormControlInput1" placeholder="">
                                </div>
                                <br>
                                    ...
                                    @foreach ($financier as $items)
                                        <tr>
                                            ...
                                            <td class="text-center d-flex">
                                                <a href="{{ route('edit_financier', $items->id) }}" class="btn btn-success">
                                                    Modifier
                                                </a>
                                                <form action="{{ route('destroy_financier', $items->id) }}" method="post">
                                                    @csrf
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Financier</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{ route('add_financier') }}" method="post" enctype="multipart/form-data">
