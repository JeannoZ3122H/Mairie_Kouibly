            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a  href="{{ route('ad_financier') }}">
                            <h3 class="title">Administratif financier</h3>
                        </a>
                    </li>
            ...
            <div class="row justify-content-center mt-3">
                @if (isset($get_administratif_financiers))
                    <div class="col-lg-6 col-md-12 col-sm-8">
                        <form action="{{ route('update_ad_financier',$get_administratif_financiers->id) }}" method="post" enctype="multipart/form-data">
                            
                            @csrf
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label for="exampleFormControlInput1" class="form-label">Responsable</label>
                                    <input type="text" value="{{ old('responsable') ?? $get_administratif_financiers->responsable }}" name="responsable" class="form-control" id="exampleFormControlInput1" placeholder="">
                                </div>
                                <br>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" name="description" id="summernote" rows="2">
                                        {{  $get_administratif_financiers->description }}
                                    </textarea>
                                </div>
                    ...
                    <div class="table-responsive">
                        @if (isset($ad_financier))
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    @foreach ($ad_financier as $items)
                                        <tr>
                                            <td>{{ $items->responsable }}</td>
                                            <td width="1000">{!! $items->description !!}</td>
                                            
                                            <td class="text-center d-flex">
                                                <a href="{{ route('edit_ad_financier', $items->id) }}" class="btn btn-success">
                                                    Modifier
                                                </a>
                                                <form action="{{ route('destroy_ad_financier', $items->id) }}" method="post">
                                                    @csrf
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Administratif financier</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{ route('add_ad_financier') }}" method="post" enctype="multipart/form-data">
